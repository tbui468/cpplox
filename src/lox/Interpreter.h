...
namespace lox {
  ...
  class Interpreter: public VisitorObject, public StmtVisitorVoid {
    ...
    private:
      ...
      std::shared_ptr<Object> visit(std::shared_ptr<This> expr)     override;
      std::shared_ptr<Object> visit(std::shared_ptr<Super> expr)     override;
      std::shared_ptr<Object> look_up_variable(const Token& name, std::shared_ptr<Expr>);
